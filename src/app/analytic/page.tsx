...
export default async function Analytic({ searchParams }: ServerComponentProps) {
  ...
  const analytic = await expenseRepository.getUserExpensesGroupedByCategories(user.id, fromTimestamp, toTimestamp)
  const notMarkedTransactions = await transactionRepository.getUserNotMakedTransactions(user.id, fromTimestamp, toTimestamp)
